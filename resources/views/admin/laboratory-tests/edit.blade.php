...
@section('title', 'Edit Laboratory Test')
                ...
                <div class="card border-0 shadow-sm">
                    <div class="card-body">
                        <div class="d-flex justify-content-between align-items-center mb-4">
                            <div>
                                <h4 class="page-title mb-1">Edit Laboratory Test</h4>
                                <p class="text-muted mb-0">Update laboratory test information</p>
                            </div>
                            <div>
                        ...
                        <form action="{{ route('laboratory-tests.update', $test->id) }}" method="POST">
                            @csrf
                            @method('PUT')
                            ...
                            <div class="row">
                                <div class="col-md-6 mb-3">
                                    ...
                                    <input type="text" class="form-control @error('name') is-invalid @enderror"
                                        id="name" name="name" value="{{ old('name', $test->name) }}"
                                ...
                                <div class="col-md-6 mb-3">
                                    ...
                                    <select class="form-select @error('sample_type') is-invalid @enderror" id="sample_type"
                                        ...
                                        @foreach ($sampleTypes as $key => $type)
                                            <option value="{{ $key }}"
                                                {{ old('sample_type', $test->sample_type) == $key ? 'selected' : '' }}>
                                                {{ $type }}
                                            </option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-6 mb-3">
                                    ...
                                    <textarea class="form-control @error('description') is-invalid @enderror" id="description" name="description"
                                        rows="4" placeholder="Enter test description (optional)">{{ old('description', $test->description) }}</textarea>
                                    @error('description')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                ...
                                <div class="col-12">
                                    <div class="d-flex gap-2">
                                        <button type="submit" class="btn btn-primary">
                                            <i class="fe fe-save me-1"></i> Update Test
                                        </button>
                                        <a href="{{ route('laboratory-tests.index') }}" class="btn btn-outline-secondary">
            ...
            <div class="col-lg-4 col-md-12">
                <div class="card border-0 shadow-sm">
                    <div class="card-body">
                        ...
                        <div class="mb-3">
                            <i class="fe fe-activity text-primary me-2"></i>
                            <strong>Sample Type:</strong> {{ $test->sample_type_badge }}
                        </div>
                        <div class="mb-3">
                            <i class="fe fe-dollar-sign text-success me-2"></i>
                            <strong>Price:</strong> {{ $test->formatted_price }}
                        </div>
                        <div class="mb-3">
                            <i class="fe fe-calendar text-info me-2"></i>
                            <strong>Created:</strong> {{ $test->created_at->format('M d, Y') }}
                        </div>
                        <div class="mb-0">
                            <i class="fe fe-clock text-warning me-2"></i>
                            <strong>Updated:</strong> {{ $test->updated_at->diffForHumans() }}
                        </div>
                    </div>
                </div>
                    ...
                    <div class="card-body">
                        ...
                        <div class="d-grid gap-2">
                            <a href="{{ route('laboratory-tests.create') }}" class="btn btn-outline-primary">
                                <i class="fe fe-plus me-2"></i>Add New Test
                            </a>
                ...
                <div class="card border-0 shadow-sm">
                    <div class="card-body">
                        ...
                        <form action="{{ route('laboratory-tests.destroy', $test->id) }}" method="POST"
                            onsubmit="return confirm('Are you sure you want to delete this laboratory test? This action cannot be undone.')">
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="btn btn-danger btn-sm">
                                <i class="fe fe-trash"></i> Delete Test
                            </button>
                        </form>
                    </div>
                </div>
