                ...
                <div class="card border-0 shadow-sm">
                    <div class="card-body">
                        <div class="d-flex justify-content-between align-items-center mb-4">
                            <div>
                                <h4 class="page-title mb-1">Edit Service Type</h4>
                                <p class="text-muted mb-0">Update service type information</p>
                            </div>
                            <div>
                        ...
                        <form method="POST" action="{{ route('service-types.update', $type->id) }}">
                                        ...
                                        <select class="form-select @error('service_category_id') is-invalid @enderror"
                                            ...
                                            @foreach ($categories as $category)
                                                <option value="{{ $category->id }}"
                                                    {{ old('service_category_id', $type->service_category_id) == $category->id ? 'selected' : '' }}>
                                                    {{ $category->name }}
                                                </option>
                                            @endforeach
                                        </select>
                            ...
                            <div class="d-flex justify-content-between">
                                ...
                                <button type="submit" class="btn btn-primary">
                                    <i class="fe fe-save me-1"></i> Update Service Type
                                </button>
                            </div>
                        </form>
                    </div>
