...
@section('title', 'Edit Drug Stock Request')
            ...
            <div class="row g-2 align-items-center">
                <div class="col">
                    <ol class="breadcrumb breadcrumb-arrows mb-1">
                        ...
                        <li class="breadcrumb-item"><a href="{{ route('drug-stock-requests.show', $request->id) }}">#{{ str_pad($request->id, 6, '0', STR_PAD_LEFT) }}</a></li>
                        <li class="breadcrumb-item active" aria-current="page">Edit</li>
                    </ol>
                    <h2 class="page-title">
                        <i class="ti-package me-2 text-primary"></i>Edit Stock Request
                    </h2>
                    <div class="text-muted mt-1">Update drug stock request details</div>
                </div>
                <div class="col-auto">
                    <a href="{{ route('drug-stock-requests.show', $request->id) }}" class="btn btn-outline-secondary">
        ...
        <div class="container-xl">
            <div class="row">
                <div class="col-lg-8">
                    <div class="card">
                        <div class="card-body">
                            <form method="POST" action="{{ route('drug-stock-requests.update', $request->id) }}">
                                @csrf
                                @method('PUT')
                                ...
                                <input type="hidden" name="requested_at"
                                    value="{{ $request->requested_at ? $request->requested_at->format('Y-m-d\TH:i') : now()->format('Y-m-d\TH:i') }}">
                                            ...
                                            <select name="program_id" class="form-select" required>
                                                ...
                                                @foreach ($programs as $program)
                                                    <option value="{{ $program->id }}"
                                                        {{ old('program_id', $request->program_id) == $program->id ? 'selected' : '' }}>
                                                        {{ $program->name }}
                                                    </option>
                                                @endforeach
                                            </select>
                                ...
                                <div class="mb-4">
                                    ...
                                    <div id="drug-requests-container">
                                        ...
                                        @if ($request->items->count() > 0)
                                                ...
                                                <div class="drug-request-item border rounded p-3 mb-3"
                                                    ...
                                                    <div class="d-flex justify-content-between align-items-center mb-3">
                                                        ...
                                                        <button type="button" class="btn btn-sm btn-danger"
                                                            ...
                                                            @if ($request->items->count() <= 1) style="display: none;" @endif>
                                                        ...
                                                        <div class="col-md-6">
                                                            <div class="mb-3">
                                                                ...
                                                                <select name="requests[{{ $index }}][drug_id]"
                                                                    class="form-select drug-select searchable-select"
                                                                    required>
                                            ...
                                            @endforeach
                                        @else
                                            <!-- Single drug fallback for existing single requests -->
                                                ...
                                                <div class="row">
                                                        ...
                                                        <div class="mb-3">
                                                            ...
                                                            <select name="requests[0][drug_id]"
                                                                ...
                                                                @foreach ($drugs as $drug)
                                                                    <option value="{{ $drug->id }}"
                                                                        data-price="{{ $drug->unit_price }}"
                                                                        {{ old('drug_id', $request->drug_id) == $drug->id ? 'selected' : '' }}>
                                                                        {{ $drug->name }} ({{ $drug->strength }}
                                                                        {{ $drug->unit }}) -
                                                                        ₦{{ number_format($drug->unit_price, 2) }}
                                                                    </option>
                                                    ...
                                                    <div class="col-md-6">
                                                        <div class="mb-3">
                                                            <label class="form-label">Priority <span
                                                                    class="text-danger">*</span></label>
                                                            <select name="requests[0][priority]" class="form-select"
                                                                required>
                                                                <option value="">Select priority...</option>
                                                                @foreach ($priorities as $key => $value)
                                                                    <option value="{{ $key }}"
                                                                        {{ old('priority', $request->priority) == $key ? 'selected' : '' }}>
                                                                        {{ $value }}
                                                                    </option>
                                                                @endforeach
                                                            </select>
                                                    ...
                                                    <div class="col-md-6">
                                                        <div class="mb-3">
                                                            ...
                                                            <input type="number" name="requests[0][quantity_requested]"
                                                                ...
                                                                min="1"
                                                                value="{{ old('quantity', $request->quantity_requested) }}">
                                                        </div>
                                                    </div>
                                                    <div class="col-md-6">
                                                        <div class="mb-3">
                                                            ...
                                                            <input type="number" name="requests[0][estimated_cost]"
                                                                class="form-control cost-input" required min="0"
                                    ...
                                    <div class="alert alert-info mt-3">
                                        <div class="d-flex justify-content-between align-items-center">
                                            ...
                                            <strong class="text-primary fs-5">₦<span
                                                    id="total-cost">{{ number_format($request->estimated_cost, 2) }}</span></strong>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="mb-3">
                                    ...
                                    <textarea name="reason" class="form-control" rows="4" required
                                        placeholder="Please explain why this stock is needed...">{{ $request->reason }}</textarea>
                                    @error('reason')
                                        <div class="invalid-feedback d-block">{{ $message }}</div>
                                    @enderror
                                </div>
                                ...
                                <div class="mb-3">
                                    ...
                                    <textarea name="notes" class="form-control" rows="3" placeholder="Any additional information...">{{ $request->notes }}</textarea>
                                    @error('notes')
                                        <div class="invalid-feedback d-block">{{ $message }}</div>
                                    @enderror
                                </div>
...
@push('scripts')
    <script>
        let drugRequestCount = {{ $request->items->count() > 0 ? $request->items->count() : 1 }};
        const drugs = @json($drugs);
        const priorities = @json($priorities);

        function addDrugRequest() {
            console.log('addDrugRequest function called');
            ...
            let requestHtml =
                ...
                '<select name="requests[' + drugRequestCount +
                '][drug_id]" class="form-select drug-select searchable-select" required>' +
                '<option value="">Select a drug...</option>';

            drugs.forEach(function(drugItem) {
                requestHtml += '<option value="' + drugItem.id + '" data-price="' + drugItem.unit_price + '">' +
                    drugItem.name + ' (' + drugItem.strength + ' ' + drugItem.unit + ') - ₦' + parseFloat(drugItem
                        .unit_price)
                    .toFixed(2) +
                    '</option>';
            });
            ...
            Object.keys(priorities).forEach(function(key) {
                requestHtml += '<option value="' + key + '">' + priorities[key] + '</option>';
            });
            ...
            requestHtml +=
                ...
                '<input type="number" name="requests[' + drugRequestCount +
                '][quantity_requested]" class="form-control quantity-input" required min="1">' +
                '</div>' +
                '</div>' +
                '<div class="col-md-6">' +
                '<div class="mb-3">' +
                ...
                '<input type="number" name="requests[' + drugRequestCount +
                '][estimated_cost]" class="form-control cost-input" required min="0" step="0.01" placeholder="Auto-calculated" readonly>' +
                '</div>' +
                '</div>' +
                '</div>' +
                '</div>';
        ...
        function attachEventListeners() {
            ...
            requestItems.forEach(function(item) {
                ...
                drugSelect.removeEventListener('change', function() {
                    updateEstimatedCost(item);
                });
                quantityInput.removeEventListener('input', function() {
                    updateEstimatedCost(item);
                });
                ...
                function filterOptions(searchTerm) {
                    ...
                    filtered.forEach(function(option) {
                        ...
                        item.addEventListener('click', function() {
                            ...
                            // Trigger change event to update cost calculation
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if (form) {
                form.addEventListener('submit', function(e) {
                    ...
                    // Also check hidden select values
                    const hiddenSelects = document.querySelectorAll('.searchable-select');
                    console.log('Hidden select values:');
                    hiddenSelects.forEach(function(select, index) {
                        console.log('Select ' + index + ' name:', select.name, 'value:', select
                            .value);
                    });
