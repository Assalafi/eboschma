...
<head>
    <meta charset="utf-8">
    <title>CRM Category Report - {{ App\Models\TicketCategory::find(request('category'))->name ?? 'Unknown' }}</title>
    <style>
...
<body>
    <!-- Header -->
    <div class="header">
        <h1>CRM Category Report</h1>
        <p>Category: {{ App\Models\TicketCategory::find(request('category'))->name ?? 'Unknown Category' }} - Generated
            on {{ date('F j, Y \a\t H:i') }}</p>
    </div>

    <!-- Filter Information -->
    <div class="filters-info">
        <strong>Filter Applied:</strong> Category =
        {{ App\Models\TicketCategory::find(request('category'))->name ?? 'Unknown Category' }}
                ...
                <div class="summary-cell">
                    <div class="summary-box">
                        <div class="summary-number">
                            {{ number_format($tickets->where('status', 'in_progress')->count()) }}</div>
                        <div class="summary-label">In Progress</div>
                    </div>
                </div>
                <div class="summary-cell">
                    <div class="summary-box">
                        <div class="summary-number">{{ number_format($tickets->where('status', 'pending')->count()) }}
                        </div>
    ...
    <div class="table-section">
        ...
        @if ($tickets->isEmpty())
            <div style="text-align: center; padding: 40px; color: #666;">
                <p style="font-size: 14px; margin: 0;">No tickets found in this category.</p>
            </div>
        @endif
    </div>
    ...
    <!-- Footer -->
    <div class="footer">
        <p>CRM Category Report - Generated by Boschma Management System</p>
